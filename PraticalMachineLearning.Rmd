...
Some variables in the original data have many invalid values (> 95%), such as "#DIV/0!" and missing values (NA). When a variable has so many these kinds of value, it would not be a good contributor to the prediction model. In this case, they are removed from the data set.  
...
Among these models, **Random Forest** takes a little longer time to train (about 40 minutes), but it has the highest prediction accuracy (**`r accuracy_RF`**) or the lowest error rate (**`r (1 - accuracy_RF)`**). Based on these, it is selected as the prediction model.  

The prediction of 20 test cases from a different data set using Random Forest is to be submitted separately.  
